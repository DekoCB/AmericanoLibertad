@php
    $mensajeWhatsapp = $whatsappHref('Hola, quiero información sobre la admisión en '.$nombreInstitucion.'.');

    $tituloLineas = [
        ['texto' => 'Creemos que la educación', 'acento' => false],
        ['texto' => 'cambia vidas', 'acento' => true],
    ];

    $letrasWordmark = str_split('LIBERTAD');
@endphp
...
{{--
    ...
    píldora "glass") que sigue al puntero. Todo vive en flujo normal dentro
    de un min-h-screen, para que el resto de la landing siga debajo con
    scroll normal.
--}}
<section
    id="inicio"
...
>
    {{-- Fondo: resplandor de respaldo + foto real del instituto + degradado que funde ambos bordes hacia el negro. --}}
    <div class="absolute inset-0 z-0">
        <div class="hero-glow absolute inset-0 bg-[radial-gradient(circle_at_20%_20%,rgba(37,99,235,0.18),transparent_45%),radial-gradient(circle_at_80%_10%,rgba(15,27,61,0.65),transparent_45%)]"></div>

        <img src="{{ asset('welcome/Port1.jpeg') }}" alt="" class="absolute inset-0 h-full w-full object-cover opacity-30">
    ...
    <div class="relative z-[2] mx-auto flex w-full max-w-4xl flex-1 flex-col items-center px-4 pb-10 pt-14 text-center sm:px-6 lg:pt-20">
        ...
        <h1 class="hero-loop-title">
            @foreach ($tituloLineas as $indiceLinea => $linea)
                ...
                @if ($abrirAcento)<span class="text-blue-400">@endif
        ...
        <p x-data x-reveal.150 class="mt-6 max-w-xl text-sm leading-relaxed text-gray-300 sm:text-base">
            {{ $nombreLargo }}: formamos profesionales preparados para afrontar los desafíos del mundo laboral mediante una enseñanza práctica, innovadora y de calidad.
        </p>

        <a x-ref="botonCta" href="{{ route('admision.create') }}" wire:navigate class="hero-loop-btn mt-8">
            <span>Solicita tu admisión</span>
            <span class="hero-loop-dot"></span>
        </a>

        <dl x-data x-reveal.200 class="mt-12 grid w-full max-w-md grid-cols-3 gap-6 border-t border-white/10 pt-8">
            <div>
                <dt class="text-xs text-gray-400">Estudiantes activos</dt>
                <dd class="mt-1 text-2xl font-bold tabular-nums text-white">{{ number_format($stats['students']) }}</dd>
            </div>
            <div>
                <dt class="text-xs text-gray-400">Docentes</dt>
                <dd class="mt-1 text-2xl font-bold tabular-nums text-white">{{ number_format($stats['teachers']) }}</dd>
            </div>
            <div>
                <dt class="text-xs text-gray-400">Cursos</dt>
                <dd class="mt-1 text-2xl font-bold tabular-nums text-white">{{ number_format($stats['subjects']) }}</dd>
            </div>
        </dl>
    </div>
    ...
    <div x-ref="pildoraCursor" class="hero-loop-pill">
        <span class="hero-loop-pill-text"><span class="is-white">Solicita tu</span> Admisión</span>
    </div>
</section>
