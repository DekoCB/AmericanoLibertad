...
new #[Layout('layouts.app')] class extends Component
{
    ...
    public function with(EvaluacionService $service): array
    {
        ...
        if ($this->cicloId) {
            ...
            $carrerasDisponibles = $delCiclo->pluck('carrera')->unique('id')->sortBy('name')->values();

            if ($this->carreraId) {
                $delCarrera = $delCiclo->where('carrera_id', $this->carreraId);
                $ciclosCurricularesDisponibles = $delCarrera->pluck('ciclo_curricular')->unique()->sort()->values();

                if ($this->cicloCurricular) {
                    $delCicloCurricular = $delCarrera->where('ciclo_curricular', $this->cicloCurricular);
                    ...
                    // idénticas.
                    $grupos = $delCicloCurricular->groupBy('curso_id');
                }
            }
        }
...
<div>
    ...
    @elseif (! $carreraId)
        ...
        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
            ...
            @empty
                <p class="col-span-full py-8 text-center text-sm text-ink-faint">Sin horarios en este ciclo.</p>
            @endforelse
        </div>
    @elseif (! $cicloCurricular)
        ...
        <div class="grid grid-cols-2 gap-4">
            ...
            @empty
                <p class="col-span-full py-8 text-center text-sm text-ink-faint">Esta carrera no tiene horarios en este ciclo.</p>
            @endforelse
        </div>
    @else
        ...
        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
            @forelse ($grupos as $grupo)
                ...
                @endif
            @empty
                <p class="col-span-full py-8 text-center text-sm text-ink-faint">Este ciclo no tiene horarios en este ciclo curricular.</p>
            @endforelse
        </div>
    @endif
</div>
